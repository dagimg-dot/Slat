...
class ClipboardService : Service() {
    ...
    private val clipboardListener =
        ClipboardManager.OnPrimaryClipChangedListener {
            if (isPanelOpen()) {
                handleClipboardChange(sourceApp = SlatAccessibilityService.currentForegroundApp)
            } else {
                SlatAccessibilityService.getInstance()?.onPrimaryClipChanged()
            }
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel =
                NotificationChannel(
                    ...
                    description = "Keeps Slat running in the background to monitor clipboard"
                    setShowBadge(false)
                }
    ...
    private fun createNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Slat is active")
